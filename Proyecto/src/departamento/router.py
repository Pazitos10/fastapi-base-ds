...
def read_departamentos(db: Session = Depends(get_db)):
    logger.info("Consultando la lista de departamentos desde endpoint...")  # <- este mensaje se verá por la terminal
...
@router.delete("/{departamento_id}", response_model=schemas.Departamento)
