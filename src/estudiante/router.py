...
def read_estudiantes(db: Session = Depends(get_db)):
    logger.info("Listando estudiantes desde router") # <- este mensaje se verá por la terminal
...
@router.delete("/{estudiante_id}", response_model=schemas.EstudianteDelete)
def delete_estudiante(estudiante_id: int, db: Session = Depends(get_db)):
    return services.eliminar_estudiante(db, estudiante_id)

@router.get("/{estudiante_id}/promedio", response_model=schemas.PromedioEstudiante, tags=["Promedio"])
def read_promedio_estudiante(estudiante_id: int, db: Session = Depends(get_db)):
    return services.obtener_promedio_calificaciones(db, estudiante_id)